...
#include "SpalartAllmarasGradientDiscretization.h"
...
//Convert low endian to big endian

/*
#include <endian.h>         
#include <algorithm> //required for std::swap

#define LowEndian2BigEndian(x) ByteSwap((unsigned char *) &x,sizeof(x))
...
class SpalartAllmarasModel<T>::Impl
{
 ...
 Impl(const GeomFields& geomFields,
  ...
  _meshes(meshes),
    ...
    _spalartAllmarasGradientModel(_meshes,_spalartAllmarasFields.nuTilde,
  ...
  void init()
  {
	...
	*oneCell = T(1.0);
	_spalartAllmarasFields.one.addArray(cells,oneCell);


	//diffusivity
	shared_ptr<TArray> diffusivityCell(new TArray(cells.getCountLevel1()));
	*diffusivityCell = T(0.0);
	_spalartAllmarasFields.diffusivity.addArray(cells,diffusivityCell);
	...
	*vorticityMagCell = T(0.0);
	...
	shared_ptr<TArray> chiCell(new TArray(cells.getCountLevel1()));
	*chiCell = T(0.);
	_spalartAllmarasFields.chi.addArray(cells,chiCell);

	// fv1
	//shared_ptr<TArray> fv1Cell(new TArray(cells.getSelfCount()));
	shared_ptr<TArray> fv1Cell(new TArray(cells.getCountLevel1()));
	*fv1Cell = T(0.);
	...
	*fv2Cell = T(0.);
	...
	// fv2
	shared_ptr<TArray> STildeCell(new TArray(cells.getSelfCount()));
	*STildeCell = T(0.);
	_spalartAllmarasFields.STilde.addArray(cells, STildeCell);
	...
	*rSACell = T(0.);
	...
	// g
	shared_ptr<TArray> gCell(new TArray(cells.getSelfCount()));
	*gCell = T(0.);
	...
	*fwCell = T(0.);
	...
	//effective viscosity
	shared_ptr<TArray> eddyViscosityCell(new TArray(cells.getCountLevel1()));
	*eddyViscosityCell = T(0.0);
	_spalartAllmarasFields.eddyViscosity.addArray(cells,eddyViscosityCell);
	...
	//sensitivity
        ...
        *sensCell = T(0.0);
        _spalartAllmarasFields.sensitivity.addArray(cells,sensCell);
	...
	*wdCell = T(0.);
	...
	//source solid for SIMP
	shared_ptr<TArray> sSolidCell(new TArray(cells.getCountLevel1()));
	*sSolidCell = T(0.);
	_spalartAllmarasFields.source.addArray(cells, sSolidCell);
	
	//nuTilda flux at faces
        foreach(const FaceGroupPtr fgPtr, mesh.getBoundaryFaceGroups())
	  {
            const FaceGroup& fg = *fgPtr;
	...
	shared_ptr<TArray> resSACell(new TArray(cells.getCountLevel1()));
	resSACell->zero();
	...
	resWDCell->zero();
	_spalartAllmarasFields.wallDistanceResidual.addArray(cells,resWDCell);
	
	
	
      }
    _spalartAllmarasFields.diffusivity.syncLocal(); //Check this. We might have to add other fields as well. 
    _niters  =0;
    _initialNorm = MFRPtr();
  }
  ...
  void ComputeDiffusivityTerm()
  {
	...
	TArray& diffusivity = dynamic_cast<TArray&>(_spalartAllmarasFields.diffusivity[cells]);
	...
	for (int i=0; i<nCells; i++)
	  {
	    diffusivity[i] = density[i]*(nu[i] + nuTilde[i])/sigma;
	  }
      }
  }
	...
	for (int i=0; i<nCells; i++)
	  {
	    ...
	    STilde[i] = max(vorticityMagnitude[i] + nuTilde[i]/(pow(kappa,2)*pow(wallDistance[i],2))*fv2[i], 0.3*vorticityMagnitude[i]);
	    //STilde[i] = vorticityMagnitude[i] + nuTilde[i]/(pow(kappa,2)*pow(wallDistance[i],2))*fv2[i];
	    r[i] = min(nuTilde[i]/(pow(kappa,T_Scalar(2.))*max(STilde[i],T_Scalar(1.0e-6))*pow(wallDistance[i],T_Scalar(2.0))),T_Scalar(10.0)); //Here might need a bound
	    //r[i] = min(nuTilde[i]/(pow(kappa,2)*STilde[i]*pow(wallDistance[i],2)),10.0);
	    g[i] = r[i] + Cw2*(pow(r[i],T_Scalar(6)) - r[i]);
	    fw[i] = g[i]*pow(((1+pow(Cw3,T_Scalar(6.)))/(pow(g[i],T_Scalar(6.0)) + pow(Cw3,T_Scalar(6.0)))),T_Scalar(1./6.));
	  }
      }    
  }
	...
	for (int n=0; n<numMeshes; n++)    
	  {
	    ...
	    for (int i=0; i<nCells; i++)
	      {
		wallDistance[i] = 1.0 - abs(cellCentroid[i][1]); 
	      }
	  }
      }
    else if (choice == 2)
	...
	for (int n=0; n<numMeshes; n++)    
	  {
	    ...
	    for (int i=0; i<nCells; i++)
	      {
		wallDistance[i] = abs(cellCentroid[i][1]); 
	      }
	  }
      }
  }
  ...
  void ComputeSourceTerm1()
  {
	...
	for (int i=0; i<nCells; i++)
	  {
	    sourceTerm1[i] = Cb1*density[i]*STilde[i];
	  }
      }
  }
  ...
  void ComputeSourceTerm2()
  {
	...
	const T Cb1 = _options["Cb1"];
	const T Cb2 = _options["Cb2"];
	const T sigma = _options["sigma"];
	const T kappa = _options["kappa"];
	const T Cw1 = Cb1/pow(kappa,2) + (1+Cb2)/sigma;

	for (int i=0; i<nCells; i++)
	  {
	    sourceTerm2[i] = -density[i]*Cw1*fw[i]/(pow(wallDistance[i],2))*nuTilde[i]; 
	  }
      }
  }
  ...
  {
    _spalartAllmarasGradientModel.compute();
    ComputeDiffusivityTerm();
    ...
    shared_ptr<Discretization>
	 ...
	 (_meshes,_geomFields,
	  ...
	  _spalartAllmarasFields.diffusivity,
	  _spalartAllmarasFields.nuTildeGradient));
    discretizations.push_back(dd);

    shared_ptr<Discretization>
      gd(new SpalartAllmarasGradientDiscretization<T>
	 (_meshes,_geomFields,
	  _spalartAllmarasFields.nuTilde,
	  _spalartAllmarasFields.nuTildeGradient,
	  _spalartAllmarasFields.density,
	  _options["Cb2"],
	  _options["sigma"]));
    discretizations.push_back(gd);
    ...
    shared_ptr<Discretization>
      sdTerm1(new SourceDiscretizationSpalartAllmarasTerm1<T,T,T>
	      (_meshes, 
	       _geomFields, 
	       _spalartAllmarasFields.nuTilde,
    ...
    shared_ptr<Discretization>
      sdTerm2(new SourceDiscretizationSpalartAllmarasTerm2<T,T,T>
	      (_meshes, 
	       _geomFields, 
	       _spalartAllmarasFields.nuTilde,
	...
	const MultiField::ArrayIndex vIndex(&_spalartAllmarasFields.nuTilde,&cells);
	TArray& rCell = dynamic_cast<TArray&>(ls.getB()[vIndex]);
	TArray& massFlux = dynamic_cast<TArray&>(_spalartAllmarasFields.convectionFlux[faces]);
	cout << "\nChecking " << endl;
      }
    */
    /////////////////////////////////////////////////////////////////////////////////
  ...
  void BoundNuTilde()
  {
	...
	TArray& nuTilde = dynamic_cast<TArray&>(_spalartAllmarasFields.nuTilde[cells]);
	for (int i=0; i<nCells; i++)
	  {
	    if (nuTilde[i] < 0)
	      nuTilde[i] = 1.e-15;
  ...
  T getNuTildeFluxIntegral(const Mesh& mesh, const int faceGroupId)
  {
    ...
    foreach(const FaceGroupPtr fgPtr, mesh.getBoundaryFaceGroups())
        ...
        if (fg.id == faceGroupId)
	  {
            const StorageSite& faces = fg.site;
            const int nFaces = faces.getCount();
            const TArray& nuTildeFlux =
              dynamic_cast<const TArray&>(_spalartAllmarasFields.nuTildeFlux[faces]);
            for(int f=0; f<nFaces; f++)
              r += nuTildeFlux[f];
            found=true;
	  }
    ...
    if (!found)
      throw CException("getNuTildeFluxIntegral: invalid faceGroupID");
    return r;
  }
  ...
  void ComputeEddyViscosity()
  {
	...
	TArray& eddyViscosity = dynamic_cast<TArray&>(_spalartAllmarasFields.eddyViscosity[cells]);
	const TArray& density = dynamic_cast<const TArray&>(_spalartAllmarasFields.density[cells]);
	const TArray& nuTilde = dynamic_cast<const TArray&>(_spalartAllmarasFields.nuTilde[cells]);
	const TArray& nu = dynamic_cast<const TArray&>(_spalartAllmarasFields.nu[cells]);
	TArray& fv1 = dynamic_cast<TArray&>(_spalartAllmarasFields.fv1[cells]);
	...
	for (int i=0; i<nCells; i++)
	  {	    
	    chi[i] = nuTilde[i]/(nu[i]);
	    fv1[i] = pow(chi[i],3)/(pow(chi[i],3) + pow(Cv1,3));
	    eddyViscosity[i] = density[i]*nuTilde[i]*fv1[i];
	  }
...
#include "SpalartAllmarasTopologyOptimization.h"
 ...
 private:
  ...
  T _objFunctionSpalartAllmarasModel;
...
template<class T>
T
SpalartAllmarasModel<T>::getNuTildeFluxIntegral(const Mesh& mesh, const int faceGroupId)
{
  return _impl->getNuTildeFluxIntegral(mesh, faceGroupId);
}

template<class T>
void
SpalartAllmarasModel<T>::ComputeEddyViscosity()
{
  _impl->ComputeEddyViscosity();
}

template<class T>
void
SpalartAllmarasModel<T>::ComputeWallDistance(const int choice)
{
  _impl->ComputeWallDistance(choice);
}
