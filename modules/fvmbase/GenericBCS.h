...
public:
  ...
  void applySymmetryBC() const
  {
    for(int f=0; f<this->_faces.getCount(); f++)
    {
        ...
        const VectorT3 en = this->_faceArea[f]/this->_faceAreaMag[f];
        const T xC0_dotn = dot(this->_x[c0],en);
        this->_r[c1] = this->_x[c0] - 2*xC0_dotn * en - this->_x[c1];
	/*
	cout << endl;
	cout <<"______________________________________________" << endl;
	cout << "c1 = " << c1 << ": r[c1] = " << this->_r[c1] << endl;
	cout << "c1 = " << c1 << ": x[c1] = " << this->_x[c1] << endl;
	cout << "c0 = " << c0 << ": x[c0] = " << this->_x[c0] << endl;
	cout << "xC0_dotn = " << xC0_dotn << endl;
	*/
...
public:
  ...
  void applySymmetryBC() const
  {
    for(int f=0; f<this->_faces.getCount(); f++)
    {
        ...
        const VectorT3 en = this->_faceArea[f]/this->_faceAreaMag[f];
        const T xC0_dotn = dot(this->_x[c0],en);
        this->_r[c1] = this->_x[c0] - 2*xC0_dotn * en;
#endif
    }
  }
};
