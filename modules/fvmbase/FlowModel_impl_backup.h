...
class FlowModel<T>::Impl
{
 ...
 Impl(const GeomFields& geomFields,
  ...
  void updateSourceFieldType1()
  {
	...
	for ( int c=0; c<nCells; c++)
	  {
	    source[c] = _options["alphaSolid"] + (_options["alphaFluid"] - _options["alphaSolid"])*beta[c]*(1+_options["p"])/(beta[c]+_options["p"]);
	  }
  ...
  void initResidual()
  {
	...
	shared_ptr<VectorT3Array> resMomentumCell(new VectorT3Array(cells.getCountLevel1()));
	VectorT3 resMom;
	resMom[0] = T(0.0);
	resMom[1] = T(0.0);
	resMom[2] = T(0.0);
	*resMomentumCell = resMom;
	
	shared_ptr<TArray> resPressureCell(new TArray(cells.getCountLevel1()));
	*resPressureCell = T(0.0);
	_flowFields.residualPressure.addArray(cells,resPressureCell);
      }  
  }

#if (defined(USING_ATYPE_RAPID))

  void initVelocityPressureField(const Field& velocityDoubleField, const Field& pressureDoubleField, const MeshList& meshesDouble)
  {
    const int numMeshes = _meshes.size();
    for (int n=0; n<numMeshes; n++)
      {
	...
	//const VectorT3<double,double,double>& velocityCellDouble = dynamic_cast<const VectorT3<double,double,double>&>(velocityDoubleField[cellsDouble]);
	//typedef VectorDouble<double,3> VectorDouble3;
  ...
  void initBetaField(const Field& betaDoubleField, const MeshList& meshesDouble)
  {
    const int numMeshes = _meshes.size();
    for (int n=0; n<numMeshes; n++)
      {
  ...
  void setIndependentVariables()
  {
	...
	for(int c=0; c<nCellsWithoutGhosts; c++)
	  {
	    betaCell[c].setIndex(c+3*nCells);
  ...
  void initMassFlux()
  {
    //Initialize the mass flux with average velocities
...
template<class T>
void
FlowModel<T>::updateMassFluxField()
{
  return  _impl->updateMassFluxField();
}
...
template<class T>
void
FlowModel<T>::updateSourceFieldType1()
{
...
template<class T>
void
FlowModel<T>::initVelocityPressureField(const Field& velocityDoubleField, const Field& pressureDoubleField, const MeshList& meshesDouble)
{
  return _impl->initVelocityPressureField(velocityDoubleField, pressureDoubleField, meshesDouble);
}
template<class T>
void
FlowModel<T>::initBetaField(const Field& betaDoubleField, const MeshList& meshesDouble)
{
  return _impl->initBetaField(betaDoubleField, meshesDouble);
}

template<class T>
void
FlowModel<T>::setIndependentVariables()
{
  _impl->setIndependentVariables();
}
#endif
