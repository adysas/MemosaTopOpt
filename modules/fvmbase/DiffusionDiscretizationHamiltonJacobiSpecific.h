...
#ifndef _DIFFUSIONDISCRETIZATIONHAMILTONJACOBOSPECIFIC_H_
#define _DIFFUSIONDISCRETIZATIONHAMILTONJACOBOSPECIFIC_H_
...
template<class X, class Diag, class OffDiag>
  class DiffusionDiscretizationHamiltonJacobiSpecific : public Discretization
{
 public:
 ...
 DiffusionDiscretizationHamiltonJacobiSpecific(const MeshList& meshes,
					       const GeomFields& geomFields,
					       Field& varField,
					       const Field& diffusivityField,
					       const Field& varGradientField,
					       const T_Scalar epsilon=T_Scalar(0.1)) :
  ...
  void discretize(const Mesh& mesh, MultiFieldMatrix& mfmatrix, MultiField& xField, MultiField& rField)
  {
    ...
    //This will only work assuming diffusion discretization is called before any other discretizations. 
    const int nCells = cells.getCountLevel1();    
    for(int c=0; c<nCells; c++)
      {
	rCell[c] = _epsilon*xCell[c]*rCell[c];
#if !(defined(USING_ATYPE_RAPID))
	//diag[c] *= _epsilon;
	diag[c] = _epsilon*xCell[c]*diag[c];
	//diag[c] = _epsilon*(xCell[c]*diag[c] + rCell[c]);
#endif
      }
