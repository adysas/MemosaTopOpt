...
#include "SourceConvectionHeatTransferDiscretization.h"
 ...
 Impl(const GeomFields& geomFields,
  ...
  void init()
  {
	...
	*oneCell = T(1.0);
	...
	//beta
        ...
        *betaCell = _options["volumeFraction"];
        _thermalFields.beta.addArray(cells,betaCell);
	
	//heat transfer coefficient
        shared_ptr<TArray> htcCell(new TArray(cells.getCountLevel1()));
        *htcCell = _options["heatTransferCoefficient"];
        _thermalFields.heatTransferCoefficient.addArray(cells, htcCell);

	//sensitivity
        shared_ptr<TArray> sensCell(new TArray(cells.getSelfCount()));
        *sensCell = T(0.0);
        _thermalFields.sensitivity.addArray(cells,sensCell);
	

	//sensitivityConstraint
        shared_ptr<TArray> sensConstraintCell(new TArray(cells.getSelfCount()));
        *sensConstraintCell = T(0.0);
        _thermalFields.sensitivityConstraint.addArray(cells,sensCell);

	//create specific heat field   rho*Cp; 
	//I have changed the specific heat field to volumetricHeatCapacity field 
	...
	*rho = vc["density"];
	...
	shared_ptr<TArray> rhocp(new TArray(cells.getCount()));
	*rhocp = vc["density"]*vc["specificHeat"];
	_thermalFields.volumetricHeatCapacity.addArray(cells, rhocp);
	...
	resMomCell->zero();
	_thermalFields.momentumResidual.addArray(cells,resMomCell);
	
	shared_ptr<TArray> resContCell(new TArray(cells.getCountLevel1()));
	resContCell->zero();
	_thermalFields.continuityResidual.addArray(cells,resContCell);
	
	shared_ptr<TArray> resTempCell(new TArray(cells.getCountLevel1()));
	resTempCell->zero();
	_thermalFields.temperatureResidual.addArray(cells,resTempCell);
	...
	shared_ptr<Discretization>
	  td(new TimeDerivativeDiscretization<T, T, T>
	     (_meshes, _geomFields, 
	      ...
	      _thermalFields.volumetricHeatCapacity,
	      _options["timeStep"]));
	discretizations.push_back(td);
      }
    
    shared_ptr<Discretization>
      schtd(new SourceConvectionHeatTransferDiscretization<T,T,T>
	 (_meshes, 
	  _geomFields, 
	  _thermalFields.temperature,
	  _thermalFields.heatTransferCoefficient,
	  _options["TemperatureAmbient"]));
    discretizations.push_back(schtd);
...
#include "ThermalTopologyOptimization.h"
...
#include "ThermalTopologyOptimizationImpl.h"
