...
class FlowModel<T>::Impl
{
 public:
  ...
  //for topopt/////
  typedef Vector<double,3> VectorDouble3;
  typedef Array<VectorDouble3> VectorDouble3Array;
  typedef DiagonalTensor<double,3> DiagTensorDouble3;
  typedef CRMatrix<DiagTensorDouble3,double,VectorDouble3> VVDoubleMatrix;
  typedef typename VVDoubleMatrix::DiagArray VVDoubleDiagArray;
  typedef Gradient<double> PDoubleGradType;
  typedef Array<PDoubleGradType> PDoubleGradArray;
 ...
 Impl(const GeomFields& geomFields,
  ...
  void init()
  {
	...
	//Pressure for each cell and each face
	...
	shared_ptr<TArray> vorticityMag(new TArray(cells.getCountLevel1()));
        *vorticityMag = T(0.0);
        _flowFields.vorticityMagnitude.addArray(cells,vorticityMag);
	...
	shared_ptr<TArray> eddyMuCell(new TArray(cells.getCountLevel1()));
        *eddyMuCell = T(0.);
        _flowFields.eddyviscosity.addArray(cells,eddyMuCell);

	shared_ptr<TArray> totalMuCell(new TArray(cells.getCountLevel1()));
        //*totalMuCell = T(0.);
	*totalMuCell = vc["viscosity"];
        _flowFields.totalviscosity.addArray(cells,totalMuCell);

	shared_ptr<TArray> wallDistanceCell(new TArray(cells.getCountLevel1()));
        *wallDistanceCell = T(0.);
        _flowFields.wallDistance.addArray(cells,wallDistanceCell);

	//Added this for Ergun/Brinkmans model
	shared_ptr<TArray> sourceCell(new TArray(cells.getCountLevel1()));
        sourceCell->zero();
	...
	//beta
        ...
        *betaCell = _options["volumeFraction"];
        _flowFields.beta.addArray(cells,betaCell);
	
	//sensitivity
        shared_ptr<TArray> sensCell(new TArray(cells.getSelfCount()));
        *sensCell = T(0.0);
        _flowFields.sensitivity.addArray(cells,sensCell);

	//////////////////////////
	// residualMomentum and residualContinuity for topology optimization
  ...
  void linearizeMomentum(LinearSystem& ls)
  {
    ...
    shared_ptr<Discretization>
      sd(new SourceDiscretizationFlowModel<VectorT3, DiagTensorT3, T>
	 (_meshes,_geomFields, 
	  _flowFields.velocity,
	  _flowFields.source));
    ...
    */
    ...
    //cout << "-----------------PREBC----------------------------------"<<endl;
    //dumpLinearSystem(ls.getMatrix(), ls.getB(), ls.getX(), "PreBC");
    //cout << "-----------------PREBC----------------------------------"<<endl;
#endif
	...
	const StorageSite& cells = mesh.getCells();
	const VectorT3Array& vel = dynamic_cast<const VectorT3Array&>(_flowFields.velocity[cells]);
	//////////////
		...
		if (bc.bcType == "PressureBoundary")
		  {
		    ...
		    fixedPressureMomentumBC(faces,mesh,ls.getMatrix(), ls.getX(), ls.getB());
#endif
		  }
	      }
	    else if ((bc.bcType == "Symmetry"))
...
#if !(defined(USING_ATYPE_TANGENT) || defined(USING_ATYPE_PC) || defined(USING_ATYPE_RAPID))
    //cout << "-----------------POSTBC----------------------------------"<<endl;
    //dumpLinearSystem(ls.getMatrix(), ls.getB(), ls.getX(), "PostBC");
    //cout << "-----------------POSTBC----------------------------------"<<endl;
#endif
  ...
  MFRPtr solveMomentum()
  {
    ...
    ls.initSolve();
#if !(defined(USING_ATYPE_TANGENT) || defined(USING_ATYPE_PC) || defined(USING_ATYPE_RAPID))
    //dumpLinearSystem(ls.getMatrix(), ls.getB(), ls.getX(), ls.getDelta(), "PostInitSolution");
#endif
...
#if !(defined(USING_ATYPE_TANGENT) || defined(USING_ATYPE_PC) || defined(USING_ATYPE_RAPID))   
    //dumpLinearSystem(ls.getMatrix(), ls.getB(), ls.getX(), ls.getDelta(), "PostSolution");
#endif
	...
	const VectorT3Array& velocityArray =
	  dynamic_cast<const VectorT3Array&>(_flowFields.velocity[cells]);
	_momApField->addArray(cells,dynamic_pointer_cast<ArrayBase>(momAp.newCopy()));
	_flowFields.momentumAp.addArray(cells,dynamic_pointer_cast<ArrayBase>(momAp.newCopy()));
	_flowFields.previousVelocity.addArray(cells,dynamic_pointer_cast<ArrayBase>(velocityArray.newCopy()));
      }
  ...
  void dumpLinearSystem(MultiFieldMatrix& matrix, MultiField& b, MultiField& x, MultiField& delta, string fileBase)
  {
#if !(defined(USING_ATYPE_TANGENT) || defined(USING_ATYPE_PC) || defined(USING_ATYPE_RAPID))
	...
	VVDiagArray& vDiag = vMatrix.getDiag();

	TArray& vCoeff = vMatrix.getOffDiag();

	const CRConnectivity& cr = vMatrix.getConnectivity();
	...
	int nCoeffs = nCells;
	int nFlatRows = nCells*dimension;
	...
	for(int i=0; i<nCells; i++)
	  {
	    ...
	    for (int j=row[i]; j<row[i+1]; j++)
	      {
		cout << i+1 << "," << col[j]+1 << "=" << vCoeff[j] << "; ";
		fprintf(matFile, "%d %d %22.15le \n", i+1, col[j]+1, vCoeff[j]); 
	      }
	    cout << endl;
	...
	VectorT3Array& deltaCell = dynamic_cast<VectorT3Array&>(delta[vIndex]);
	VectorT3Array& xCell = dynamic_cast<VectorT3Array&>(x[vIndex]);

	
	string rhsFileName = fileBase + ".rhs";
	string resFileName = fileBase + ".res";
	FILE *rhsFile = fopen(rhsFileName.c_str(),"wb");
	FILE *resFile = fopen(resFileName.c_str(),"wb");
    
	for(int i=0; i<nCells; i++)
	  {
	    fprintf(rhsFile,"%d %le \t %le \t %le \t %le \t %le \t %le\n", i+1, -rCell[i][0], -rCell[i][1], deltaCell[i][0], deltaCell[i][1], xCell[i][0], xCell[i][1]);
	    fprintf(resFile,"%22.15le\n", -rCell[i][0]);
	  }
	...
	/*
	  ...
	  VectorT3Array& rFace = dynamic_cast<VectorT3Array&>(b[fIndex]);
	  //VectorT3Array& deltaFace = dynamic_cast<VectorT3Array&>(delta[fIndex]);
	  //VectorT3Array& xFace = dynamic_cast<VectorT3Array&>(x[fIndex]);
  ...
  void dumpLinearSystem(MultiFieldMatrix& matrix, MultiField& b, MultiField& x, string fileBase)
  {
#if !(defined(USING_ATYPE_TANGENT) || defined(USING_ATYPE_PC) || defined(USING_ATYPE_RAPID))
	...
	for(int i=0; i<nCells; i++)
	  {
	    fprintf(rhsFile,"%d %le \t %le \t %le \t %le\n", i+1, -rCell[i][0], -rCell[i][1], xCell[i][0], xCell[i][1]);
	  }
...
#if !(defined(USING_ATYPE_TANGENT) || defined(USING_ATYPE_PC) || defined(USING_ATYPE_RAPID))    
        //cout << "----------------- Interior faces ----------------------------------"<<endl;
	//preDumpPressureLinearSystem(matrix, b, "Before");
	//cout << "----------------- Interior faces ----------------------------------"<<endl;
#endif
	// interior
	netFlux += discretizeMassFluxInterior(mesh,iFaces,matrix,x,b);

	//This is temporary for debugging to check whether the computed massfluxes after momentum interpolation are right 
	/////////////////////////////////////////
        const StorageSite& allFaces = mesh.getFaces();	
	TArray& massFlux = dynamic_cast<TArray&>(_flowFields.massFlux[allFaces]);

	/////////////////////////////////////////
	
#if !(defined(USING_ATYPE_TANGENT) || defined(USING_ATYPE_PC) || defined(USING_ATYPE_RAPID))
        //cout << "----------------- Interior faces ----------------------------------"<<endl;
	//preDumpPressureLinearSystem(matrix, b, "IntFaces");
	//cout << "----------------- Interior faces ----------------------------------"<<endl;
#endif
	// interfaces
	foreach(const FaceGroupPtr fgPtr, mesh.getInterfaceGroups())
	...
	foreach(const FaceGroupPtr fgPtr, mesh.getBoundaryFaceGroups())
	  {
		...
		netFlux += fixedFluxContinuityBC(faces,mesh,matrix,x,b,bc);
	      ...
	      {                      		
		netFlux += fixedPressureContinuityBC(faces,mesh,matrix,x,b,bc);
  ...
  void postContinuitySolve(LinearSystem& ls)
  {
	...
	/*
	{
	  const StorageSite& allFaces = mesh.getFaces();
	  TArray& massFlux = dynamic_cast<TArray&>(_flowFields.massFlux[allFaces]);
	  cout << "\Before post continuity solve " << endl;
	}
	*/
#if !(defined(USING_ATYPE_RAPID))        
	...
	foreach(const FaceGroupPtr fgPtr, mesh.getBoundaryFaceGroups())
	  {
	    ...
	    /*
	    {
	      const StorageSite& allFaces = mesh.getFaces();
	      TArray& massFlux = dynamic_cast<TArray&>(_flowFields.massFlux[allFaces]);
	      cout << "\nAfter post continuity solve " << endl;
	    }
	    */
  ...
  MFRPtr solveContinuity()
  {
    ...
    //dumpPressureLinearSystem(*ls, "Continuity");
    //cout << "----------------- Post Pressure ----------------------------------"<<endl;
#endif
...
#if !(defined(USING_ATYPE_TANGENT) || defined(USING_ATYPE_PC) || defined(USING_ATYPE_RAPID))
    //cout << "----------------- Post Cleap Up ----------------------------------"<<endl;
    //dumpPressureLinearSystem(*ls, "CleanUp");
    //cout << "----------------- Post Clean Up ----------------------------------"<<endl;
#endif
    postContinuitySolve(*ls);

#if !(defined(USING_ATYPE_TANGENT) || defined(USING_ATYPE_PC) || defined(USING_ATYPE_RAPID))
    //cout << "----------------- Post Pressure ----------------------------------"<<endl;
    //dumpPressureLinearSystem(*ls, "Pressure");
    //cout << "----------------- Post Pressure ----------------------------------"<<endl;
  ...
  void dumpPressureLinearSystem(LinearSystem& ls, string fileBase)

  {
	...
	//Cell variables
	TArray& rCell = dynamic_cast<TArray&>(b[pIndex]);
	TArray& deltaCell = dynamic_cast<TArray&>(delta[pIndex]);
	TArray& xCell = dynamic_cast<TArray&>(x[pIndex]);
	...
	for(int i=0; i<nCells; i++)
	  {
	    fprintf(rhsFile,"%d %le \t %le \t %le \n", i+1, -rCell[i], deltaCell[i], xCell[i]);
	    fprintf(resFile,"%22.15le\n", -rCell[i]);
	  }
  ...
  void preDumpPressureLinearSystem(MultiFieldMatrix& matrix, MultiField& b, string fileBase)
	...
	if (fg.id == faceGroupId)
	  {
	    ...
	    const int nFaces = faces.getCount();
	    const CRConnectivity& faceCells = mesh.getFaceCells(faces);
	    const CRConnectivity& faceAllCells = mesh.getAllFaceCells();
	    const CRConnectivity& cellFaces = mesh.getCellFaces();
	    const TArray& massFlux = dynamic_cast<const TArray&>(_flowFields.massFlux[faces]);	
	    int nFacesReverseFlow = 0;
	    cout << endl;
	    for(int f=0; f<nFaces; f++)
	      {
		const int c1 = faceCells(f,1);
		const int gf = cellFaces(c1,0);
		if (massFlux[f] < 0.0)
		  {
		    nFacesReverseFlow++;
		    cout << "Reverse flow in face: " << f << ", Massflux = " << massFlux[f] << " Cell# " << c1 << "GlobalFace# " << gf << endl; 
		  }		
	      }	 
	    if (nFacesReverseFlow > 0)
	      cout << "Total number of faces with reverse flow = " << nFacesReverseFlow << endl;
	  }      
      }
  }  
...
#include "FlowModelTurbulence.h"
#include "FlowTopologyOptimization.h"
 

  /*
    #include "FlowFilter.h"
    #include "ThermalFlowLaminarTopologyOptimization.h"
  */
...
template<class T>
void
FlowModel<T>::advanceMomentum()
{
  return _impl->advanceMomentum();
}

template<class T>
void
FlowModel<T>::advanceContinuity()
{
  return _impl->advanceContinuity();
}
...
template<class T>  
void
FlowModel<T>::checkReverseFlow(const Mesh& mesh, const int faceGroupId)
{
  _impl->checkReverseFlow(mesh, faceGroupId);
}


/*
  #include "FlowTopologyOptimizationImpl.h"
  #include "ThermalFlowLaminarTopologyOptimizationImpl.h"
  #include "FlowFilterImpl.h"
*/

#include "FlowModelTurbulenceImpl.h"
#include "FlowTopologyOptimizationImpl.h"
